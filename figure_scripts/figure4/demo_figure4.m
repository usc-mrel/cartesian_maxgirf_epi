...
for idx = 1:N3
    ...
    img_dicom_gnc0(:,:,idx) = RescaleSlope * dicomread(dicom_info).' + RescaleIntercept; % transpose it!
end
...
for idx = 1:N3
    ...
    img_dicom_gnc1(:,:,idx) = RescaleSlope * dicomread(dicom_info).' + RescaleIntercept; % transpose it!
end

%% Set the size of a variable
Nx = N1;
Ny = N2;
...
%% Read a .cfl file
img_maxgirf_gnc0 = complex(zeros(Nkx, Nky, nr_slices, 'single')); % Gridding/PHC/CFC/SFC/GNC = 1/1/1/0/0
img_maxgirf_gnc1 = complex(zeros(Nkx, Nky, nr_slices, 'single')); % Gridding/PHC/CFC/SFC/GNC = 1/1/1/0/1
...
for slice_number = 1:nr_slices
    ...
    %----------------------------------------------------------------------
    img1_filename1 = sprintf('img_maxgirf_PCLR_slc%d_wgt1_dir1_gridding1_phc1_conc1_gnl0_static0_flat_i6_l0.00', slice_number);
    ...
    img_maxgirf_gnc0(:,:,actual_slice_number) = readcfl(cfl_file);
    fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));

    %----------------------------------------------------------------------
    % Gridding/PHC/CFC/SFC/GNC = 1/1/1/0/1
    %----------------------------------------------------------------------
    img1_filename2 = sprintf('img_maxgirf_PCLR_slc%d_wgt1_dir1_gridding1_phc1_conc1_gnl1_static0_flat_i6_l0.00', slice_number);
    cfl_file = fullfile(output_path2, img1_filename2);
    tstart = tic; fprintf('%s:(SLC=%2d/%2d) Reading a .cfl file: %s... ', datetime, slice_number, nr_slices);
    img_maxgirf_gnc1(:,:,actual_slice_number) = readcfl(cfl_file);
    fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
end
...
for slice_number = 11%1:nr_slices
    ...
    %----------------------------------------------------------------------
    ax1 = subplot(2,2,1);
    imagesc(ax1, abs(img_maxgirf_gnc0(:,:,slice_number)).');
    colormap(ax1, gray(256));
    axis image off;
    xlim(ax1, xlimits);
    ylim(ax1, ylimits);
    text(ax1, N1/ 2, 6 * 3, '(Phase-const.) Cartesian MaxGIRF', 'Color', 'k', 'Interpreter', 'tex', 'FontSize', FontSize, 'FontWeight', 'bold', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
    text(ax1, N1/ 2, 6 * 4, 'corrections applied {\color[rgb]{0.9255 0.3882 0.0}during recon}', 'Rotation', 0, 'Color', 'k', 'Interpreter', 'tex', 'FontSize', FontSize, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
    text(ax1, 26, N2 / 2, {'w/ concomitant field correction', 'w/o GNL correction'}, 'Rotation', 90, 'Color', 'k', 'Interpreter', 'tex', 'FontSize', FontSize, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
    text(ax1, 150, 10, '2D SE-EPI: axial, b0 image, 1.45 x 1.45 mm^2, R = 2, PF = 6/8, ETL = 65, 4 NSA', 'Color', blue, 'Interpreter', 'tex', 'FontSize', FontSize + 2, 'FontWeight', 'bold', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
    text(ax1, 29, 25, '(A)', 'FontSize', 16, 'FontWeight', 'Bold', 'Color', 'w', 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left');

    % Arrow   % left-right: N2 / 2
    ...
    text(ax1, ylimits(1) + 3, N2 / 2, sprintf('PE direction (A >> P)'), 'FontSize', FontSize, 'Rotation', -90, 'Interpreter', 'tex', 'Color', 'r', 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'center');

    %----------------------------------------------------------------------
    % MaxGIRF + PCLR: Gridding/PHC/CFC/SFC/GNC = 1/1/1/0/1
    %----------------------------------------------------------------------
    ax3 = subplot(2,2,3);
    imagesc(ax3, abs(img_maxgirf_gnc1(:,:,slice_number)).');
    colormap(ax3, gray(256));
    axis image off;
    xlim(ax3, xlimits);
    ylim(ax3, ylimits);
    text(ax3, 26, N2 / 2, {'w/ concomitant field correction', 'w/ GNL correction'}, 'Rotation', 90, 'Color', 'k', 'Interpreter', 'tex', 'FontSize', FontSize, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
    text(ax3, 29, 25, '(C)', 'FontSize', 16, 'FontWeight', 'Bold', 'Color', 'w', 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left');

    %----------------------------------------------------------------------
    % DICOM: Gridding/PHC/CFC/SFC/GNC = 1/1/1/0/0
    %----------------------------------------------------------------------
    ax2 = subplot(2,2,2);
    imagesc(ax2, img_dicom_gnc0(:,:,slice_number).');
    colormap(ax2, gray(256));
    axis image off;
    xlim(ax2, xlimits);
    ylim(ax2, ylimits);
    text(ax2, N1/ 2, 6 * 3, 'Traditional reconstruction', 'Color', 'k', 'Interpreter', 'tex', 'FontSize', FontSize, 'FontWeight', 'bold', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
    text(ax2, N1/ 2, 6 * 4, 'corrections applied {\color[rgb]{0.0118 0.6000 0.6000}after recon}', 'Rotation', 0, 'Color', 'k', 'Interpreter', 'tex', 'FontSize', FontSize, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
    text(ax2, 29, 25, '(B)', 'FontSize', 16, 'FontWeight', 'Bold', 'Color', 'w', 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left');
    %text(ax2, N2 / 2, ylimits(1), sprintf('slice at z = %4.2f mm', z(1,1,slice_number) * 1e3), 'FontSize', FontSize, 'Rotation', 0, 'Interpreter', 'latex', 'Color', 'r', 'VerticalAlignment', 'top', 'HorizontalAlignment', 'center');

    %----------------------------------------------------------------------
    % DICOM: Gridding/PHC/CFC/SFC/GNC = 1/1/1/0/1
    %----------------------------------------------------------------------
    ax4 = subplot(2,2,4);
    imagesc(ax4, img_dicom_gnc1(:,:,slice_number).');
    colormap(ax4, gray(256));
    axis image off;
    xlim(ax4, xlimits);
    ylim(ax4, ylimits);
    text(ax4, 29, 25, '(D)', 'FontSize', 16, 'FontWeight', 'Bold', 'Color', 'w', 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left');
    %text(ax4, N1/ 2, 6 * 1, 'Vendor reconstruction', 'Color', 'k', 'Interpreter', 'tex', 'FontSize', 16, 'FontWeight', 'bold', 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
    %text(ax4, N1/ 2, 6 * 2, 'w/ concomitant field correction ({\color[rgb]{0.0118 0.6000 0.6000}after recon})', 'Color', 'k', 'Interpreter', 'tex', 'FontSize', FontSize, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');
    %text(ax4, N1/ 2, 6 * 3, 'w/ GNL correction ({\color[rgb]{0.0118 0.6000 0.6000}after recon})', 'Color', 'k', 'Interpreter', 'tex', 'FontSize', FontSize, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom');

    set(ax1, 'Position', [0.1746 0.4613 0.4124 0.4468]);
    set(ax2, 'Position', [0.5280 0.4613 0.4124 0.4468]);
    set(ax3, 'Position', [0.1746 0.0129 0.4124 0.4468]);
    ...
    %----------------------------------------------------------------------
    % Create arrow
    annotation(gcf, 'arrow', [0.539893617021277 0.518617021276596],...
        [0.665258711721225 0.662090813093981], 'Color', [1 0 0], 'LineWidth', 2,...
        'HeadStyle', 'plain');

    % Create arrow
    annotation(gcf, 'arrow', [0.539893617021277 + 0.353 0.518617021276596 + 0.353],... % left-right location
        [0.665258711721225 0.662090813093981], 'Color', [1 0 0], 'LineWidth', 2,...
        'HeadStyle','plain');

    % Create arrow
    annotation(gcf, 'arrow', [0.539893617021277 0.518617021276596],...
        [0.665258711721225 - 0.45 0.662090813093981 - 0.45], 'Color', [1 0 0], 'LineWidth', 2,...
        'HeadStyle', 'plain');

    % Create arrow
    annotation(gcf, 'arrow', [0.539893617021277 + 0.353 0.518617021276596 + 0.353],... % location
        [0.665258711721225 - 0.45 0.662090813093981 - 0.45], 'Color', [1 0 0], 'LineWidth', 2,...
        'HeadStyle', 'plain');
    ...
    %----------------------------------------------------------------------
    % Create arrow
    annotation(gcf, 'arrow', [0.316489361702128 0.295212765957447],...
        [0.595564941921856 0.592397043294612], 'Color', color_order{3}, 'LineWidth', 2,...
        'HeadStyle', 'plain');

    % Create arrow
    annotation(gcf, 'arrow', [0.316489361702128 + 0.353 0.295212765957447 + 0.353],...
        [0.595564941921856 0.592397043294612], 'Color', color_order{3}, 'LineWidth', 2,...
        'HeadStyle', 'plain');

    % Create arrow
    annotation(gcf, 'arrow', [0.316489361702128 0.295212765957447],...
        [0.595564941921856 - 0.45 0.592397043294612 - 0.45], 'Color', color_order{3}, 'LineWidth', 2,...
        'HeadStyle', 'plain');
    ...
    export_fig(sprintf('figure4_slc%d', slice_number), '-r300', '-tif', '-c[20, 300, 20, 560]'); % [top,right,bottom,left]
    close gcf;
end
