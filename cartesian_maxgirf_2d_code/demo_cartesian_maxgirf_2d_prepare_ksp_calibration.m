% demo_cartesian_maxgirf_2d_prepare_ksp_calibration.m
% Written by Nam Gyun Lee
% Email: namgyunl@usc.edu, ggang56@gmail.com (preferred)
% Started: 01/13/2025, Last modified: 02/10/2025
...
%% Get navigator data for parallel calibration
nav_data = raw_data.select(find(acq_is_navigation_data & acq_is_parallel_calibration));

%% Get calibration data
cal_data = raw_data.select(find(~acq_is_noise_measurement & acq_is_parallel_calibration & ~acq_is_navigation_data));

%% Check if the data contains a calibration dataset
if cal_data.getNumber == 0
    return;
end
...
%--------------------------------------------------------------------------
number_of_samples       = double(max(cal_data.head.number_of_samples));
discard_pre             = double(max(cal_data.head.discard_pre));
discard_post            = double(max(cal_data.head.discard_post));
center_sample           = double(max(cal_data.head.center_sample));
nr_channels             = double(max(cal_data.head.active_channels));
nr_phase_encoding_steps = double(max(cal_data.head.idx.kspace_encode_step_1)) + 1;
nr_slice_encoding_steps = double(max(cal_data.head.idx.kspace_encode_step_2)) + 1;
nr_averages             = double(max(cal_data.head.idx.average)) + 1;
nr_slices               = double(max(cal_data.head.idx.slice)) + 1;
nr_contrasts            = double(max(cal_data.head.idx.contrast)) + 1;
nr_phases               = double(max(cal_data.head.idx.phase)) + 1;
nr_repetitions          = double(max(cal_data.head.idx.repetition)) + 1;
nr_sets                 = double(max(cal_data.head.idx.set)) + 1;
nr_segments             = double(max(cal_data.head.idx.segment)) + 1;

%--------------------------------------------------------------------------
% Get the dimensionality of the trajectory vector (0 means no trajectory)
%--------------------------------------------------------------------------
trajectory_dimensions = double(max(cal_data.head.trajectory_dimensions));
...
%--------------------------------------------------------------------------
% Get a list of calibration acquisitions
%--------------------------------------------------------------------------
cal_acq_list = find((cal_data.head.idx.slice == 0));

%--------------------------------------------------------------------------
% Get a list of segments (calibration)
%--------------------------------------------------------------------------
cal_segment_list = double(cal_data.head.idx.segment(cal_acq_list));
...
%--------------------------------------------------------------------------
% Set the number of phase-encoding steps
%--------------------------------------------------------------------------
Ni = length(cal_acq_list);
...
phase_dir = double(cal_data.head.phase_dir(:,1));
read_dir  = double(cal_data.head.read_dir(:,1));
slice_dir = double(cal_data.head.slice_dir(:,1));
...
%--------------------------------------------------------------------------
% Correct the EPI factor (i.e., echo train length)
%--------------------------------------------------------------------------
etl = Ni;
...
phase_encoding_line_number_list = double(cal_data.head.idx.kspace_encode_step_1(cal_acq_list)) - (kspace_encoding_step_1_maximum - kspace_encoding_step_1_center + 1) + floor(Nky/2) + 1;

%% Calculate a list of partition encoding numbers (starting from 1)
partition_encoding_number_list = double(cal_data.head.idx.kspace_encode_step_2(cal_acq_list)) - (kspace_encoding_step_2_maximum - kspace_encoding_step_2_center + 1) + floor(Nkz/2) + 1;
...
for eco = 1:etl
    ...
    %----------------------------------------------------------------------
    if cal_segment_list(eco) == 0 % forward (segment = 0)
        echo_sign = +1;
...
if cal_segment_list(1) == 0 % forward (segment = 0)
    echo_sign = +1;
else % reverse (segment == 1)
    echo_sign = -1;
end
...
for shot_number = 1:nr_shots
    ...
    if partition_encoding_number_list_per_shot(1) == 0
        partition_encoding_dephaser_area(shot_number) = 0;
    else
        % [rad/m] / ([rad/sec/T] * [T/1e3mT]) => [mT/m*sec]
...
%--------------------------------------------------------------------------
cfl_file = fullfile(output_path, 't_cal_grt');
...
%--------------------------------------------------------------------------
cfl_file = fullfile(output_path, 'g_cal_gcs_grt_nominal');
tstart = tic; fprintf('%s: Writing a .cfl file: %s... ', datetime, cfl_file);
...
%--------------------------------------------------------------------------
cfl_file = fullfile(output_path, 'k_cal_grt');
tstart = tic; fprintf('%s: Writing a .cfl file: %s... ', datetime, cfl_file);
...
%--------------------------------------------------------------------------
cfl_file = fullfile(output_path, 't_cal_adc');
tstart = tic; fprintf('%s: Writing a .cfl file: %s... ', datetime, cfl_file);
...
%--------------------------------------------------------------------------
cfl_file = fullfile(output_path, 'k_cal_adc');
tstart = tic; fprintf('%s: Writing a .cfl file: %s... ', datetime, cfl_file);
...
%% Read navigator data (1 x Nkx x nr_navigators x nr_channels x ones(1,6) x (11)nr_repetitions x ones(1,2) x (14)nr_slices)
%--------------------------------------------------------------------------
...
%--------------------------------------------------------------------------
%--------------------------------------------------------------------------
% Calculate the number of navigator data per slice
%--------------------------------------------------------------------------
nav_acq_list = find((nav_data.head.idx.slice == 0) & (nav_data.head.idx.repetition == 0));
nr_navigators = length(nav_acq_list);

%--------------------------------------------------------------------------
% Preallocate navigator data
%--------------------------------------------------------------------------
ksp_cal_nav = complex(zeros([Nkx nr_navigators Nc nr_repetitions nr_slices], 'single'));
...
for idx = 1:nr_recons
    ...
    %----------------------------------------------------------------------
    % Get a list of navigator acquisitions for the current slice
    %----------------------------------------------------------------------
    nav_acq_list = find((nav_data.head.idx.slice      == (slice_number - 1)) & ...
                        (nav_data.head.idx.repetition == (repetition_number - 1)));

    %----------------------------------------------------------------------
    % Sort one acquisition at a time
    %----------------------------------------------------------------------
    nr_acquisitions = length(nav_acq_list);
    for i = 1:nr_acquisitions
        tstart = tic; fprintf('%s:(SLC=%2d/%2d)(REP=%2d/%2d)(i=%d/%d) Reading "navigator" k-space data... ', datetime, slice_number, nr_slices, repetition_number, nr_repetitions, i, nr_acquisitions);
        ksp_cal_nav(:,i,:,repetition_number,slice_number) = nav_data.data{nav_acq_list(i)};
        fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
    end
end
...
%--------------------------------------------------------------------------
ksp_cal_nav = reshape(ksp_cal_nav, [1 Nkx nr_navigators Nc ones(1,6) nr_repetitions ones(1,2) nr_slices]);

%% Write a .cfl file
%--------------------------------------------------------------------------
% ksp_cal_nav (1 x Nkx x nr_navigators x nr_channels x ones(1,6) x nr_repetitions x ones(1,2) x nr_slices)
%--------------------------------------------------------------------------
cfl_file = fullfile(output_path, 'ksp_cal_nav');
tstart = tic; fprintf('%s: Writing a .cfl file: %s... ', datetime, cfl_file);
writecfl(cfl_file, ksp_cal_nav);
fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
...
% -h      help
%--------------------------------------------------------------------------
ksp_nav_file    = strcat(bart_output_path, 'ksp_cal_nav');
ksp_nav_cc_file = strcat(bart_output_path, 'ksp_cal_nav_cc');
...
%% Read a .cfl file
cfl_file = fullfile(output_path, 'ksp_cal_nav_cc');
tstart = tic; fprintf('%s: Reading a .cfl file: %s... ', datetime, cfl_file);
ksp_cal_nav_cc = readcfl(cfl_file);
fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
ksp_cal_nav_cc = reshape(ksp_cal_nav_cc, [Nkx nr_navigators 1 nr_repetitions nr_slices]);
...
for shot_number = 1:nr_shots
    ...
    %----------------------------------------------------------------------
    if sum(partition_encoding_number_list_per_shot(:)) == 0
        mask(:,phase_encoding_line_number_list_per_shot,:) = true;
    else
...
for idx = 1:nr_recons
    ...
    if nr_contrasts > 1
        description = sprintf('%s_eco%d', description, contrast_number);
    end

    if nr_phases > 1
        description = sprintf('%s_pha%d', description, phase_number);
    end

    if nr_repetitions > 1
        description = sprintf('%s_rep%d', description, repetition_number);
    end

    if nr_sets > 1
        description = sprintf('%s_set%d', description, set_number);
    end
    ...
    %----------------------------------------------------------------------
    cal_acq_list = find((cal_data.head.idx.slice      == (slice_number - 1))      & ...
                        (cal_data.head.idx.contrast   == (contrast_number - 1))   & ...
                        (cal_data.head.idx.phase      == (phase_number - 1))      & ...
                        (cal_data.head.idx.repetition == (repetition_number - 1)) & ...
                        (cal_data.head.idx.set        == (set_number - 1)));
    ...
    %----------------------------------------------------------------------
    nav_acq_list = find((nav_data.head.idx.slice      == (slice_number - 1))      & ...
                        (nav_data.head.idx.contrast   == (contrast_number - 1))   & ...
                        (nav_data.head.idx.phase      == (phase_number - 1))      & ...
                        (nav_data.head.idx.repetition == (repetition_number - 1)) & ...
                        (nav_data.head.idx.set        == (set_number - 1)));
    ...
    if nav_segment_list(1) == 0 % forward (segment = 0)
        ...
        %------------------------------------------------------------------
        S1_forward = ksp_cal_nav_cc(:,1,1,repetition_number,slice_number);
        S3_forward = ksp_cal_nav_cc(:,3,1,repetition_number,slice_number);
        ...
        %------------------------------------------------------------------
        S2_reverse = ksp_cal_nav_cc(:,2,1,repetition_number,slice_number);

    else % reverse (segment = 1)
        %------------------------------------------------------------------
        % Get the first and third echo signals, S1+ and S3+
        %------------------------------------------------------------------
        S1_reverse = ksp_cal_nav_cc(:,1,1,repetition_number,slice_number);
        S3_reverse = ksp_cal_nav_cc(:,3,1,repetition_number,slice_number);
        ...
        % Get the second echo signal, S2-
        %------------------------------------------------------------------
        S2_forward = ksp_cal_nav_cc(:,2,1,repetition_number,slice_number);
    end
    ...
    title({sprintf('Phase difference (calibration data)'), sprintf('SLC = %d, Gridding = %d, %d voxels', slice_number, gridding_flag, nr_voxels)}, 'Interpreter', 'latex', 'FontWeight', 'normal', 'FontSize', 12);
    legend('Raw', 'Linear least-squares fit', 'Interpreter', 'latex', 'FontSize', 10);
    export_fig(fullfile(output_path, sprintf('phase_difference_cal_slc%d_gridding%d', slice_number, gridding_flag)), '-r300', '-tif', '-c[0, 140, 20, 100]'); % [top,right,bottom,left]
    close gcf;

    %% Get "Cartesian-format" "calibration" k-space data (Nkx x Nky x Nkz x Nc)
    %----------------------------------------------------------------------
    % ( 1)not used   x ( 2)readout dimension x ( 3)number of TRs  x ( 4)COIL_DIM
    % ( 5)MAPS_DIM   x ( 6)TE_DIM            x ( 7)COEFF_DIM      x ( 8)COEFF2_DIM
    % ( 9)ITER_DIM   x (10)CSHIFT_DIM        x (11)TIME_DIM       x (12)TIME2_DIM
    ...
    %----------------------------------------------------------------------
    ksp_cal_odd_cartesian  = complex(zeros(Nkx, Nky, Nkz, Nc, 'single'));
    ksp_cal_even_cartesian = complex(zeros(Nkx, Nky, Nkz, Nc, 'single'));

    %----------------------------------------------------------------------
    % Sort one acquisition at a time
    %----------------------------------------------------------------------
    nr_acquisitions = length(cal_acq_list);

    for i = 1:nr_acquisitions
        tstart = tic; fprintf('%s:(SLC=%2d/%2d)(i=%3d/%3d) Reading "calibration" k-space data... ', datetime, slice_number, nr_slices, i, nr_acquisitions);

        %------------------------------------------------------------------
        % Get the phase encoding line number
        %------------------------------------------------------------------
        kspace_encode_step_1 = double(cal_data.head.idx.kspace_encode_step_1(cal_acq_list(i)));
        ...
        %------------------------------------------------------------------
        kspace_encode_step_2 = double(cal_data.head.idx.kspace_encode_step_2(cal_acq_list(i)));
        ...
        %------------------------------------------------------------------
        readout = cal_data.data{cal_acq_list(i)}; % number_of_samples x Nc
        ...
        %------------------------------------------------------------------
        if gridding_flag
            if cal_segment_list(i) == 0 % forward (segment = 0)
        ...
        %------------------------------------------------------------------
        if phc_flag
            if cal_segment_list(i) == 1 % reverse (segment = 1)
                %----------------------------------------------------------
                % Apply forward FFT to move from k-space to image space
                % Siemens: k-space <=> image space
                %----------------------------------------------------------
        ...
        % Accumulate k-space
        %------------------------------------------------------------------
        if cal_segment_list(i) == cal_segment_list(1) % odd
            ksp_cal_odd_cartesian(kx_range,phase_encoding_line_number,partition_encoding_number,:) = ksp_cal_odd_cartesian(kx_range,phase_encoding_line_number,partition_encoding_number,:) + reshape(readout, [Nkx 1 1 Nc]);
        elseif cal_segment_list(i) == cal_segment_list(2) % even
            ksp_cal_even_cartesian(kx_range,phase_encoding_line_number,partition_encoding_number,:) = ksp_cal_even_cartesian(kx_range,phase_encoding_line_number,partition_encoding_number,:) + reshape(readout, [Nkx 1 1 Nc]);
        end
        fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
    end

    %% Combine odd and even k-space data
    ksp_cal_cartesian = ksp_cal_odd_cartesian + ksp_cal_even_cartesian;
    ...
    %% Add the relative table position
    %pcs_offset = pcs_offset - [relative_table_position.x relative_table_position.y relative_table_position.z].' * 1e-3;

    %% Get a slice offset in the PCS from ISMRMRD format
    sag_offset_ismrmrd = double(cal_data.head.position(1,cal_acq_list(1))); % [mm]
    cor_offset_ismrmrd = double(cal_data.head.position(2,cal_acq_list(1))); % [mm]
    tra_offset_ismrmrd = double(cal_data.head.position(3,cal_acq_list(1))); % [mm]
    pcs_offset_ismrmrd = [sag_offset_ismrmrd; cor_offset_ismrmrd; tra_offset_ismrmrd] * 1e-3; % [mm] * [m/1e3mm] => [m]

    %% Get a rotation matrix from the GCS to the PCS (ISMRMRD format)
    phase_dir = double(cal_data.head.phase_dir(:,cal_acq_list(1)));
    read_dir  = double(cal_data.head.read_dir(:,cal_acq_list(1)));
    slice_dir = double(cal_data.head.slice_dir(:,cal_acq_list(1)));
    R_gcs2pcs_ismrmrd = [phase_dir read_dir slice_dir];
    ...
    if ~exist(strcat(cfl_file, '.cfl'), 'file')
        tstart = tic; fprintf('%s:(SLC=%2d/%2d) Calculating concomitant field basis functions... ', datetime, slice_number, nr_slices);
    ...
    if cfc_flag
        if ~exist(strcat(cfl_file, '.cfl'), 'file')
            ...
            U = single(u_tilde(:,1:Lmax)); % Nk x Lmax
        ...
        S = cat(1, 1, zeros(Lmax - 1, 1, 'single'));
    ...
    if ~exist(strcat(cfl_file, '.cfl'), 'file')
        ...
        U_time_reversed = complex(zeros(Nkx, Nky, Nkz, Lmax, 'single'));
        ...
        for shot_number = 1:nr_shots
            ...
            %--------------------------------------------------------------
            cal_segment_list_per_shot = cal_segment_list(shot_range);
            ...
            for eco = 1:etl
                ...
                if partition_encoding_number == 0, partition_encoding_number = partition_encoding_number + 1; end

                %----------------------------------------------------------
                % Flip a "reverse" line in the left singular vectors
                %----------------------------------------------------------
                if cal_segment_list_per_shot(eco) == 0 % forward (segment == 0)
                    U_time_reversed(:,phase_encoding_line_number,partition_encoding_number,:) = U(:,eco,:);
                else % reverse (segment == 1)
                    U_time_reversed(:,phase_encoding_line_number,partition_encoding_number,:) = flip(U(:,eco,:),1);
                end
            end
        end
    ...
    tstart = tic; fprintf('%s: Writing a .cfl file: %s... ', datetime, cfl_file);
    writecfl(cfl_file, ksp_cal_cartesian);
    fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
    ...
    if ~exist(strcat(cfl_file, '.cfl'), 'file')
        tstart = tic; fprintf('%s: Writing a .cfl file: %s... ', datetime, cfl_file);
        writecfl(cfl_file, U_time_reversed);
        fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
    end
