% demo_cartesian_maxgirf_2d_recon_phase_constrained_lowrank.m
...
for slice_number = 1:nr_slices
    ...
    ksp = complex(zeros(Nkx, Nky, Nkz, Nc, nr_repetitions, 'single'));

    for repetition_number = 1:nr_repetitions
        %------------------------------------------------------------------
        ...
        ksp(:,:,:,:,repetition_number) = readcfl(cfl_file);
        fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
    end

    %% Read a "low-resolution" reconstructed image
    img_lowres = complex(zeros(Nkx, Nky, Nkz, 1, nr_repetitions, 'single'));

    for repetition_number = 1:nr_repetitions
        %------------------------------------------------------------------
        % img_maxgirf_lowres (Nkx x Nky x Nkz)
        %------------------------------------------------------------------
        cfl_file = fullfile(output_path, sprintf('img_maxgirf_lowres_slc%d_rep%d_gridding%d_phc%d_cfc%d_sfc%d_gnc%d_%s_i%d_l%4.2f', slice_number, repetition_number, gridding_flag, phc_flag, cfc_flag, sfc_flag, gnc_flag, slice_type, maxiter, lambda));
        tstart = tic; fprintf('%s: Reading a .cfl file: %s... ', datetime, cfl_file);
        img_lowres(:,:,:,:,repetition_number) = readcfl(cfl_file);
        fprintf('done! (%6.4f/%6.4f sec)\n', toc(tstart), toc(start_time));
    end
...
%     ev_mask = (ev_maps > 0.98); % 0.94 for in-vivo
%     ev_mask2 = bwareaopen(ev_mask, 600); % Keep only blobs with an area of 300 pixels or more.
%     se = strel('disk',10);
%     ev_mask_eroded = imerode(ev_mask2,se);
%     ev_mask_dilated = imdilate(ev_mask_eroded,se);
...
%     sens = bsxfun(@times, sens, ev_mask_dilated);
    ...
    tstart = tic; fprintf('%s:(SLC=%2d/%2d) Performing Cartesian MaxGIRF + phase-constrained low-rank reconstruction:\n', datetime, slice_number, nr_slices);
    ...
    fig_filename = sprintf('img_maxgirf_pclr_slc%d_gridding%d_phc%d_cfc%d_sfc%d_gnc%d_%s_i%d_l%4.2f', slice_number, gridding_flag, phc_flag, cfc_flag, sfc_flag, gnc_flag, slice_type, maxiter, lambda);
